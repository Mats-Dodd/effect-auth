...
layer(AuthTest.layerHttp(served))("http/Users", (it) => {
  ...
  it.effect("does not serve the callback to a browser with no session", () =>
    Effect.gen(function* () {
      const { client } = yield* makeClient()
      const refused = yield* Effect.flip(client.auth.deleteUserCallback({ query: { token: "nonsense" } }))
      assert.strictEqual(refused._tag, "Unauthorized")
    })
  )
  ...
  it.layer(
    ...
    it.effect("refuses the fourth change-email attempt in a window", () =>
      Effect.gen(function* () {
        ...
        const limited = yield* Effect.flip(client.auth.changeEmail({ payload: { newEmail: uniqueEmail("limited-4") } }))
