...
describe("crypto/PasswordHasher", () => {
  describe("scrypt", () => {
    it.effect("round-trips a password", () =>
      Effect.gen(function*() {
    ...
    // only test that pays for real parameters, and it is worth it.
  ...
  describe("cross-format verification", () => {
    it.effect("verifies a pbkdf2 hash under the scrypt layer", () =>
      Effect.gen(function*() {
        ...
        assert.strictEqual(yield* Effect.provide(hasher.use((_) => _.verify(password, hash)), scrypt), true)
        assert.strictEqual(yield* Effect.provide(hasher.use((_) => _.verify(wrong, hash)), scrypt), false)
      }))
    ...
    it.effect("verifies a scrypt hash under the pbkdf2 layer", () =>
      Effect.gen(function*() {
        ...
        assert.strictEqual(yield* Effect.provide(hasher.use((_) => _.verify(password, hash)), pbkdf2), true)
        assert.strictEqual(yield* Effect.provide(hasher.use((_) => _.verify(wrong, hash)), pbkdf2), false)
      }))
    ...
    it.effect("verifies with the cost the hash was written at, not today's", () =>
      Effect.gen(function*() {
        ...
        assert.strictEqual(yield* Effect.provide(hasher.use((_) => _.verify(password, legacy)), scrypt), true)
      }))
  })
  ...
  describe("timingSafeEqualUint8", () => {
    it("compares every byte regardless of where the difference is", () => {
