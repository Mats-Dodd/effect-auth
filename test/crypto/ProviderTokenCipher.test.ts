...
it.effect("crypto/ProviderTokenCipher authenticates ciphertext and its storage location", () =>
  Effect.gen(function* () {
    ...
    assert.strictEqual(yield* cipher.decrypt("account-a", "access_token", encrypted), "provider-secret")

    const copiedToAnotherAccount = yield* Effect.exit(cipher.decrypt("account-b", "access_token", encrypted))
    const copiedToAnotherField = yield* Effect.exit(cipher.decrypt("account-a", "refresh_token", encrypted))
...
it.effect("crypto/ProviderTokenCipher rejects modified and legacy plaintext values", () =>
  Effect.gen(function* () {
    ...
    assert.strictEqual(Exit.isFailure(yield* Effect.exit(cipher.decrypt("account-a", "id_token", tampered))), true)
    assert.strictEqual(
