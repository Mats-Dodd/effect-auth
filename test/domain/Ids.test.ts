...
layer(layerWebCrypto)("domain/Ids/uuidV7 layout", (it) => {
  it.effect("writes the version and variant bits and the big-endian timestamp", () =>
    // The block shares one `TestClock`, so a test that moves time takes one of
    // its own rather than dragging its siblings forward with it.
    AuthTest.freshClock(
      Effect.gen(function* () {
...
layer(layerOnes)("domain/Ids/uuidV7 layout (all-ones randomness)", (it) => {
...
layer(layerWebCrypto)("domain/Ids/uuidV7", (it) => {
