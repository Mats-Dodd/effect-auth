...
const adminFor = (url: string): Effect.Effect<SqlClient.SqlClient, SqlError.SqlError> => {
  ...
  const created = perWorker(
    Effect.provide(PgClient.make({ url: Redacted.make(url), maxConnections: 2 }), Reactivity.layer)
  )
...
const database = (serverUrl: Effect.Effect<string>) =>
  Effect.gen(function* () {
    ...
    return yield* Effect.provide(
      PgClient.make({ url: Redacted.make(withDatabase(url, name)), maxConnections: 4 }),
      Reactivity.layer
    )
  })
